{{--
<div x-data="{ isYouTube: false }" x-init="
    console.log('currentItem dans _video.blade.php:', currentItem);
    isYouTube = currentItem.mediaSrc.includes('youtube.com') || currentItem.mediaSrc.includes('youtu.be');
">
    ...
    <template x-if="!isYouTube">
        <video class="w-full max-h-[80vh]" controls>
...
<div x-data="{
    ...
    <video
        ...
        preload="metadata"
        :poster="poster"
        class="object-cover relative z-10 w-full h-full bg-black"
    ...
    <div x-show="videoPlayerReady" class="absolute inset-0 w-full h-full">
        <div x-ref="videoBackground" @click="togglePlay()" class="flex absolute inset-0 z-30 justify-center items-center w-full h-full bg-black cursor-pointer group">
            <div
                ...
                class="flex absolute z-20 justify-center items-center w-24 h-24 bg-blue-600 rounded-full opacity-0"
                x-cloak>
                <svg class="w-10 h-10 text-white translate-x-0.5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8.42737 3.41611C6.46665 2.24586 4.00008 3.67188 4.00007 5.9427L4 18.0572C3.99999 20.329 6.46837 21.7549 8.42907 20.5828L18.5698 14.5207C20.4775 13.3802 20.4766 10.6076 18.568 9.46853L8.42737 3.41611Z" fill="currentColor"></path></svg>
            </div>
            <div
            ...
            <div class="absolute z-10 duration-300 ease-out group-hover:scale-110">
                <button
                    ...
                    class="flex justify-center items-center w-12 h-12 text-white rounded-full duration-150 ease-out cursor-pointer bg-blue-600/80" type="button">
        ...
        <div x-show="controls"
             ...
             class="absolute top-0 left-0 z-20 w-full h-1/4 bg-gradient-to-b from-black to-transparent opacity-20" x-cloak>
        </div>
        <div x-show="controls"
             x-transition:enter="transition ease-out duration-300"
             x-transition:enter-start="translate-y-full"
             ...
             x-transition:leave-start="translate-y-0"
             x-transition:leave-end="translate-y-full"
             class="absolute bottom-0 left-0 z-20 w-full h-1/4 bg-gradient-to-b from-transparent to-black opacity-20" x-cloak>
        </div>
        <div x-show="controls"
             ...
             x-transition:leave-start="translate-y-0"
             x-transition:leave-end="-translate-y-full"
             class="flex absolute top-0 left-0 z-40 items-center w-full h-12 text-white" x-cloak>
            <div class="flex absolute top-0 right-0 items-center mt-0.5 mr-0.5">
                <div class="flex items-center h-auto group">
                    <button @click="toggleMute()" type="button" class="flex justify-center items-center w-6 h-auto opacity-80 duration-150 ease-out hover:opacity-100">
                    ...
                    <div class="relative invisible mx-0 w-0 h-1.5 rounded-full duration-300 ease-out group-hover:mx-1 group-hover:w-12 group-hover:visible">
                        <input
                            x-ref="volume"
                            ...
                            class="w-full h-full appearance-none flex items-center cursor-pointer bg-transparent z-30
                                ...
                                [&::-webkit-slider-runnable-track]:bg-white/30 [&::-webkit-slider-runnable-track]:rounded-full [&::-webkit-slider-runnable-track]:overflow-hidden [&::-moz-range-track]:bg-neutral-200 [&::-moz-range-track]:rounded-full [&::-ms-track]:bg-neutral-200 [&::-ms-track]:rounded-full
                                [&::-moz-range-progress]:bg-white/80 [&::-moz-range-progress]:rounded-full [&::-ms-fill-lower]:bg-white/80 [&::-ms-fill-lower]:rounded-full [&::-webkit-slider-thumb]:shadow-[-995px_0px_0px_990px_rgba(255,_255,_255,_0.8)]
                        ">
                    </div>
                </div>
                <button x-ref="fullscreenButton" @click="handleFullscreen" class="flex justify-center items-center w-10 h-10 opacity-80 duration-150 ease-out scale-90 hover:opacity-100 hover:scale-100" type="button">
        ...
        <div x-show="controls"
            ...
            <div class="absolute bottom-0 z-30 px-2.5 w-full -translate-y-8">
            ...
            <div class="flex absolute bottom-0 left-0 z-20 items-center w-full h-8 text-white">

                <div x-show="showTime" class="flex justify-between items-center mx-3 w-full font-mono text-xs opacity-80 hover:opacity-100" x-cloak>
